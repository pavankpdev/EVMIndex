...
export const prepareContract = (config: Contract[]): Listener[] => {
    return config.map((contract) => {
        ...
        const ABI = require(`${workerData?.ABIPath}/${source.abi}.json`)
        ...
        return {
            ...
            events: eventHandlers.map((eventHandler: EventHandler, i): EventHandler => {
                ...
                const ABI = require(`${workerData?.ABIPath}/${contract.source.abi}.json`)
                ...
                const handler: Record<string, any> = require(`${workerData?.handlerPath}/${eventHandler.file}`);
                return {
